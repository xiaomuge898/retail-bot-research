...
| 数据 | 数量 | 用途 |
|---|---:|---|
| `样本1` 至 `样本9` | 135 组 | 分析新结构、开发算法和校准参数 |
| `验证样本` | 35 组 | 参数冻结后的回归验证 |
| 合计 | 170 组 | 每组包含一张 JPG 背景图、一张 PNG 小拼图和对应标签 |
...
已有验证报告记录 35/35 组验证样本误差不超过 3 px，最大绝对误差为 3 px，平均绝对误差为 1.057 px。样本9为 15/15 组通过，最大绝对误差为 2 px。该结果只代表压缩包内现有分布，不应推断为所有未知平台都能达到相同精度。
