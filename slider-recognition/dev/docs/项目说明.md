...
当前实现按图片结构组合 alpha 蒙版、饱和度、边界梯度、多阈值 Canny 和候选聚类。多个独立特征在相近位置重复命中时，结果通常比单一阈值稳定。
...
[`dev/小拼图 + 背景图 样本.zip`](../小拼图%20+%20背景图%20样本.zip) 是算法适配数据，不是程序运行时的强制依赖。压缩包包含：

| 数据 | 数量 | 用途 |
|---|---:|---|
| `样本1` 至 `样本8` | 120 组 | 分析新结构、开发算法和校准参数 |
| `验证样本` | 30 组 | 参数冻结后的回归验证 |
| 合计 | 150 组 | 每组包含一张 JPG 背景图、一张 PNG 小拼图和对应标签 |

每个样本目录中的 `滑块真实距离.txt` 保存图片编号与期望横坐标。开发集可以用于调试；验证集只用于最终回归，避免根据验证答案反复调参。

### 解压步骤

适配代码或运行 `dev/tests/` 下的完整测试前，必须先解压。以下脚本仅使用 Python 标准库，并主动拒绝覆盖已有目录：

```python
from pathlib import Path
from zipfile import ZipFile

archive = Path("dev/小拼图 + 背景图 样本.zip")
extracted = archive.parent / archive.stem
target = archive.parent / "样本"

if extracted.exists() or target.exists():
    raise FileExistsError("解压目录已存在，请确认内容后再处理")

with ZipFile(archive) as package:
    package.extractall(archive.parent)

# 测试代码固定从 dev/样本/ 读取数据，因此统一目录名。
...
也可以使用系统解压工具，但解压后仍需把顶层的 `小拼图 + 背景图 样本` 重命名为 `样本`。
...
已有验证报告记录 30/30 组验证样本误差不超过 3 px，最大绝对误差为 3 px，平均绝对误差为 1.133 px。该结果只代表压缩包内现有分布，不应推断为所有未知平台都能达到相同精度。
